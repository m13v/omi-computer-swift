...
export class ClientSideConnection {
        ...
        const handler = async (method, params) => {
            switch (method) {
                ...
                case schema.CLIENT_METHODS.terminal_release: {
                    ...
                    return client.releaseTerminal?.(validatedParams);
                }
                ...
                case schema.CLIENT_METHODS.terminal_kill: {
                    const validatedParams = schema.killTerminalRequestSchema.parse(params);
                    return client.killTerminal?.(validatedParams);
                }
                default:
                    throw RequestError.methodNotFound(method);
            }
    ...
    async loadSession(params) {
        await this.#connection.sendRequest(schema.AGENT_METHODS.session_load, params);
    }
    /**
            ...
            for (const line of lines) {
                ...
                if (trimmedLine) {
                    ...
                    try {
                        ...
                        console.error("Unexpected error during message processing:", trimmedLine, err);
                        if (id) {
                            this.#sendMessage({
                                jsonrpc: "2.0",
                                id,
