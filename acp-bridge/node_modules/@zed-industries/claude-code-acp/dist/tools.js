...
export function toolInfoFromToolUse(toolUse, cachedFileContent) {
    ...
    switch (name) {
        ...
        case "MultiEdit": {
            ...
            let oldTextMulti = multiInput.edits.map((edit) => edit.old_string).join("\n");
            let newTextMulti = multiInput.edits.map((edit) => edit.new_string).join("\n");
            try {
                if (multiInput.edits && multiInput.file_path) {
                    const oldContent = cachedFileContent[multiInput.file_path] ||
                        multiInput.edits.map((edit) => edit.oldText).join("\n");
                    const newContent = replaceAndCalculateLocation(oldContent, multiInput.edits.map((edit) => ({
            ...
            if (toolResult.content && toolResult.content.length > 0) {
                try {
                    ...
                    if (data.lineNumbers && Array.isArray(data.lineNumbers)) {
                        const locations = data.lineNumbers.map((line) => ({
