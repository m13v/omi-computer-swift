        ...
        while (true) {
            ...
            switch (message.type) {
                ...
                case "assistant": {
                    ...
                    for (const notification of toAcpNotifications(message, params.sessionId, this.toolUseCache, this.fileContentCache)) {
...
async function availableSlashCommands(query) {
    ...
    return commands
        ...
        .filter((command) => !(command.name.match(/\(MCP\)/) || UNSUPPORTED_COMMANDS.includes(command.name)));
}
 ...
 */
export function toAcpNotifications(message, sessionId, toolUseCache, fileContentCache) {
    const chunks = message.message.content;
    const output = [];
    ...
    for (const chunk of chunks) {
        ...
        switch (chunk.type) {
            ...
            case "tool_result": {
                ...
                if (!toolUse) {
                    console.error(`[claude-code-acp] Got a tool result for tool use that wasn't tracked: ${chunk.tool_use_id}`);
                    break;
                }
