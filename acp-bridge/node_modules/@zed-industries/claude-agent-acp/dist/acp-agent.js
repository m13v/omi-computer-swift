...
import { toolInfoFromToolUse, planEntries, toolUpdateFromToolResult, toolUpdateFromEditToolResponse, registerHookCallback, createPostToolUseHook, } from "./tools.js";
...
export class ClaudeAcpAgent {
    ...
    async prompt(params) {
        ...
        while (true) {
            ...
            switch (message.type) {
                ...
                case "stream_event": {
                    for (const notification of streamEventToAcpNotifications(message, params.sessionId, this.toolUseCache, this.client, this.logger, { clientCapabilities: this.clientCapabilities })) {
                ...
                case "assistant": {
                    ...
                    if (typeof message.message.content === "string" &&
                        ...
                        if (message.message.content.includes("Context Usage")) {
                            for (const notification of toAcpNotifications(message.message.content
                                ...
                                .replace("</local-command-stdout>", ""), "assistant", params.sessionId, this.toolUseCache, this.client, this.logger, { clientCapabilities: this.clientCapabilities })) {
                                await this.client.sessionUpdate(notification);
                            }
                        }
                    ...
                    for (const notification of toAcpNotifications(content, message.message.role, params.sessionId, this.toolUseCache, this.client, this.logger, { clientCapabilities: this.clientCapabilities })) {
                        await this.client.sessionUpdate(notification);
                    }
                    break;
                }
    ...
    async setSessionConfigOption(params) {
        ...
        session.configOptions = session.configOptions.map((o) => o.id === params.configId ? { ...o, currentValue: params.value } : o);
    ...
    async replaySessionHistory(sessionId, filePath) {
        ...
        try {
            for await (const line of reader) {
                ...
                for (const notification of toAcpNotifications(content, role, sessionId, toolUseCache, this.client, this.logger, { registerHooks: false, clientCapabilities: this.clientCapabilities })) {
                    await this.client.sessionUpdate(notification);
                }
            }
        }
            ...
            if (toolName === "ExitPlanMode") {
                const response = await this.client.requestPermission({
                    ...
                    toolCall: {
                        ...
                        rawInput: toolInput,
                        ...toolInfoFromToolUse({ name: toolName, input: toolInput, id: toolUseID }, supportsTerminalOutput),
                    },
                });
    ...
    async updateConfigOption(sessionId, configId, value) {
        ...
        if (!session)
            return;
        session.configOptions = session.configOptions.map((o) => o.id === configId ? { ...o, currentValue: value } : o);
        await this.client.sessionUpdate({
            sessionId,
            update: {
    ...
    async createSession(params, creationOpts = {}) {
        ...
        if (params._meta?.disableBuiltInTools === true) {
            ...
            disallowedTools.push("Read", "Write", "Edit", "Bash", "Glob", "Grep", "Task", "TodoWrite", "ExitPlanMode", "WebSearch", "WebFetch", "SlashCommand", "Skill", "NotebookEdit");
        }
        const options = {
            systemPrompt,
...
export function streamEventToAcpNotifications(message, sessionId, toolUseCache, client, logger, options) {
    ...
    switch (event.type) {
        case "content_block_start":
            return toAcpNotifications([event.content_block], "assistant", sessionId, toolUseCache, client, logger, { clientCapabilities: options?.clientCapabilities });
        case "content_block_delta":
            return toAcpNotifications([event.delta], "assistant", sessionId, toolUseCache, client, logger, { clientCapabilities: options?.clientCapabilities });
        // No content
        case "message_start":
        case "message_delta":
        case "message_stop":
