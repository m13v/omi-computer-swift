...
async function preWarmSession(cwd, models) {
    ...
    try {
        ...
        await Promise.all(toWarm.map(async (warmModel) => {
            try {
                const sessionParams = {
                    cwd: warmCwd,
...
async function handleQuery(msg) {
    ...
    try {
        ...
        if (err instanceof AcpError && (err.code === -32000 || err.code === -32603)) {
            ...
            logErr(`Query failed with auth/internal error (code=${err.code}), requesting re-authentication`);
            isInitialized = false;
            send({ type: "auth_required", methods: authMethods });
            return;
        }
