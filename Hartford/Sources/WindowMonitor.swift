...
class WindowMonitor {
    ...
    /// Get both the active app name and window ID (static version)
    static func getActiveWindowInfoStatic() -> (String?, CGWindowID?) {
        return ScreenCaptureService.getActiveWindowInfo()
    }

    /// Instance method for getting active window info
    func getActiveWindowInfo() -> (String?, CGWindowID?) {
